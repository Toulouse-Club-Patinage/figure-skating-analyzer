...
If the count differs, do **not** proceed — the PDF layout changed and the output is untrustworthy. Re-inspect with `pdftotext -layout backend/scripts/sources/2786-SinglesPairs-SOV-2026-27.pdf - | less`.
...
def test_element_maxima_sum_to_total(rules):
    ...
    for cat, c in rules["categories"].items():
        for seg_key, seg in c["segments"].items():
            ...
            parts += 1 if seg.get("has_duo_element") else 0
...
```typescript
import { getBaseElements } from "../sov-calculator";
