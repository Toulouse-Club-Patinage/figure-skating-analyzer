...
def test_quints_allowed_only_in_isu_free_skating(rules):
    """Règle ISU 612 governs both Senior and Junior free skating and draws no
    Senior/Junior distinction: "n'importe quel nombre de tours"."""
    allowed = {
        (cat, seg_key)
...
`quads_allowed` false → **true** and add `"quints_allowed": true` (règle ISU 612 governs the free
program for Senior and Junior alike; the Junior quad restriction belongs to the short program,
règle ISU 611), `"euler_allowed": true`, `"requires_choreo_spin": true`.
